...
/* See also MultiDrawExample::_drawType, _stagingBuffers and
   _shaderStorageBuffers for more information */
enum class DrawType {
    ...
    MultiDraw,
    /** @todo toggles for using a single material (avoiding dynamic material
        uniform load, option to have material ID match draw ID (avoiding a
        dependent load potentially), option to have draw offset but materials
        with bind offset (again avoiding one dynamic uniform load) */
};
...
class MultiDrawExample: public Platform::Application {
    ...
    private:
        ...
        /* Toggles driven by the UI, defaults picked from command line
           arguments. See the DrawType enum for details of individual types. */
        DrawType _drawType;
        #ifndef MAGNUM_TARGET_WEBGL
        /* If enabled, GPU buffer uploads are first done to a staging buffer
           visible to both CPU and GPU memory, and then copied to a GPU-local
           buffer. May result in faster uniform access on discrete GPUs, on
           unified memory GPUs the staging buffer likely won't help and the
           cost of the extra copy may slow down the rendering. */
        bool _stagingBuffers;
        /* Use SSBOs instead of UBOs. They don't have a 65k upper limit,
           meaning it's possible to draw much larger scenes without having to
           split the draws (and materials) into 65kB chunks. With UBOs only up
           to 1024 draws is possible in this example. */
        /** @todo implement UBO splitting here to allow drawing larger scenes
            without SSBOs */
        bool _shaderStorageBuffers;
        #endif
        ...
        Matrix4
            ...
            _cameraTransformation{Matrix4::translation(Vector3::zAxis(50.0f))},
...
using Magnum::Examples::DrawType;

/* For --draw-type argument parsing */
template<> struct ConfigurationValue<DrawType> {
    static DrawType fromString(Containers::StringView value, ConfigurationValueFlags) {
        #define _c(value_) if(value == #value_) return DrawType::value_;
        _c(SceneGraph)
        _c(TrivialLoop)
        _c(DeduplicatedLoop)
        _c(DeduplicatedLoopMeshViews)
        _c(UboPerDraw)
        _c(UboBindOffset)
        _c(UboDrawOffset)
        _c(MultiDraw)
        ...
        Fatal{} << "Unrecognized --draw-type" << value << Debug::nospace;
    }
};
        ...
        .setTitle("Magnum Multi Draw Example")}
{
    ...
    args
        ...
        #endif
        .addOption("draw-type", "SceneGraph").setHelp("draw-type", "pick a concrete draw type on startup", "TYPE")
        #ifndef MAGNUM_TARGET_WEBGL
        .addBooleanOption("staging-buffers").setHelp("staging-buffers", "use staging uniform buffers")
        .addBooleanOption("shader-storage-buffers").setHelp("shader-storage-buffers", "use SSBOs instead of UBOs")
        #endif
        .addBooleanOption("no-profile").setHelp("no-profile", "don't enable profiler on startup")
        .addSkippedPrefix("magnum", "engine-specific options")
        .setGlobalHelp(R"(Displays a scene, allowing to pick between various alternative drawing
approaches.

)"
#ifndef CORRADE_TARGET_EMSCRIPTEN
R"(The -I / --importer option can be also used to pass a path to an importer
plugin binary, for example to load a custom format not known to Magnum
otherwise.

)"
#endif
R"(The --draw-type option accepts one of the following values: SceneGraph,
TrivialLoop, DeduplicatedLoop, DeduplicatedLoopMeshViews, UboPerDraw,
UboBindOffset, UboDrawOffset, MultiDraw.)")
        .parse(arguments.argc, arguments.argv);

    /* Get the draw type from command-line. Useful if the default SceneGraph is
       too slow or it would ruin profiling. */
    _drawType = args.value<DrawType>("draw-type");
    #ifndef MAGNUM_TARGET_WEBGL
    _stagingBuffers = args.isSet("staging-buffers");
    _shaderStorageBuffers = args.isSet("shader-storage-buffers");
    #endif
    ...
    /* Create the UI */
    {
        ...
        /* Profiler toggle, reflect the --no-profile command line flag in the
           initial state */
        Ui::BitStorage profile{_ui, DirectInit, args.isSet("no-profile") ? false : true};
        profile->onUpdate([&](bool enabled) {
            /* If the profiler is enabled, clear the output text */
        ...
        drawType->onUpdate([&](const DrawType type) {
            /* Every time the draw type changes the profiler needs to be reset
               to not display stale numbers. Don't enable if it's not running
               though. */
            ...
            /* Show the UBO limit label if there is, if a draw type affected by
               the limit is picked, and SSBOs are disabled (or not available).
               Abusing opacity instead of the hidden flag so it doesn't cause
               UI to jump around if the label is longer. */
            if(_uboLimitLabel != Ui::NodeHandle::Null)
                _ui.setNodeOpacity(_uboLimitLabel,
                    (type == DrawType::UboDrawOffset ||
                     type == DrawType::MultiDraw)
                        #ifndef MAGNUM_TARGET_WEBGL
        ...
        #ifndef MAGNUM_TARGET_WEBGL
        Ui::BitStorage stagingBuffers{_ui, Ui::NonOwned, _stagingBuffers};
        Ui::BitStorage shaderStorageBuffers{_ui, Ui::NonOwned, _shaderStorageBuffers};
        /* Every time these change the profiler also needs to be reset to not
           display stale numbers. Don't enable if it's not running though. */
        stagingBuffers->onUpdate([&](bool) {
            if(_profiler.isEnabled())
                _profiler.enable();
        });
        shaderStorageBuffers->onUpdate([&](bool enabled) {
            if(_profiler.isEnabled())
                _profiler.enable();

            /* Show the UBO limit label if there is, if a draw type affected by
               the limit is picked, and SSBOs are disabled. Same as in the draw
               type toggle above. */
            if(_uboLimitLabel != Ui::NodeHandle::Null)
                _ui.setNodeOpacity(_uboLimitLabel,
                    (_drawType == DrawType::UboDrawOffset ||
                     _drawType == DrawType::MultiDraw) && !enabled
                     ? 1.0f : 0.0f);
        });
        #endif
        ...
        Ui::SnapLayoutRowTop info = Ui::SnapLayout::snapRoot(_ui, Ui::Snap::Top|Ui::Snap::FillX);
        {
            ...
            Ui::checkbox(column.child(), profile, "Enable profiling");
            _profilerOutput = Ui::Label(column.child(), {});
        } {
            /* Spacer between the left and right label */
            info.child(Ui::Snap::FillX);
        } {
            /* Calculate total triangle count across all draws to show in the
               stats */
            UnsignedInt drawnTriangles = 0;
            for(Containers::Pair<UnsignedInt, Containers::Pair<UnsignedInt, Int>> meshMaterial: meshesMaterials)
                drawnTriangles += _direct.meshes[meshMaterial.second().first()]->count()/3;
            ...
            Ui::label(column.child(), Utility::format(
                ...
                drawnTriangles), Text::Alignment::MiddleLeft);
            ...
            if(meshesMaterials.size() > 1024)
                _uboLimitLabel = Ui::label(column.child(), "UBOs limited to 1024 draws", Text::Alignment::MiddleLeft, Ui::LabelStyle::Danger);
        }
        ...
        Ui::SnapLayoutRow toggles1 = toggles.child();
        {
            ...
            Ui::checkbox(
                ...
                >() ?
                    Ui::NodeFlags{} : Ui::NodeFlag::Disabled),
                stagingBuffers, "Staging uniform buffers");
            Ui::checkbox(
                row.child({},
                    #ifndef MAGNUM_TARGET_GLES
                    ...
                    #endif
                    Ui::NodeFlags{} : Ui::NodeFlag::Disabled),
                shaderStorageBuffers, "SSBOs instead of UBOs");
            #endif
        }
    }
...
void MultiDrawExample::drawEvent() {
    ...
    _profiler.endFrame();
    /** @todo have some helper similar to printStatistics() that executes
        something every N frames */
...
void MultiDrawExample::pointerMoveEvent(PointerMoveEvent& event) {
    ...
    if(!_previousPosition.isZero() && event.isPrimary() && (event.pointers() & (Pointer::MouseLeft|Pointer::Finger))) {
        ...
        _manipulatorTransformation =
            Matrix4{Quaternion::rotation(_previousPosition, currentPosition).toMatrix()}*
            _manipulatorTransformation;
